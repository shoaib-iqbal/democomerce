class HomeController < ApplicationController
	def index
		...
		if Admin::Category.second.products.present? 
			...
			@category_two= Admin::Category.fourth.products.order("RANDOM()").limit(9)  rescue '' 
		...
		if Admin::Category.third.products.present? 
			...
			@category_three= Admin::Category.fifth.products.order("RANDOM()").limit(9)  rescue ''
		...
		if Admin::Category.first.products.present? 
			...
			@category_one= Admin::Category.sixth.products.order("RANDOM()").limit(9) rescue ''
